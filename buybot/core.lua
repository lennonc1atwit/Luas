local items = {
    primarys = {"ak47", "aug", "awp", "famas", "g3sg1", "galilar", "m4a1", "m4a1_silencer", "scar20", "sg556", "ssg08", "sawedoff", "nova", "mag7", "negev", "m249", "bizon", "mac10", "mp5sd", "mp7", "mp9", "p90", "ump45"},
    secondarys = {"deagle", "revolver", "elite", "fn57", "glock", "tec9", "hkp2000", "p250", "usps_silencer", "cz75a"},
    grenades = {"decoy", "flashbang", "hegrenade", "incgrenade", "molotov", "smokegrenade"},
    utility = {"defuser ", "vest", "vesthelm", "taser"}
}

local local_player = entities.GetLocalPlayer()
local COND_BOUGHT_THIS_ROUND = false
...
local indent = 60
--                                     x                                y       w                                h        desc
TransformGuiObject(ignoreSecondary,    x,                               164,   (width / 2) - padding,            h, "Buy a secondary even if you have one.")
TransformGuiObject(ignorePrimary,      1 * (width / 3),                 164,   (width / 2) - padding,            h, "Buy a primary even if you have one.")
...
local function GoShopping()
    ...
    for i = 1, 62 do
        local weapon = local_player:GetPropEntity("m_hMyWeapons", i)
    ...
    local COND_IS_PISTOL_ROUND = tonumber(local_player:GetProp("m_iAccount")) <= tonumber(client.GetConVar("mp_startmoney"))
    local COND_SHOULD_BUY_PRIMARY =
        (not COND_IS_PISTOL_ROUND and not COND_HAS_PRIMARY) or (COND_HAS_PRIMARY and ignorePrimary:GetValue())
    local COND_SHOULD_BUY_SECONDARY =
        (COND_HAS_SECONDARY and not ignoreSecondary:GetValue()) or ignoreSecondary:GetValue()
    ...
    if COND_IS_PISTOL_ROUND then
        utilityList = altUtility
    end
...
callbacks.Register("FireGameEvent", function(event)
    if event then
        ...
        if event_name == "round_prestart" then
            COND_BOUGHT_THIS_ROUND = false
            if local_player:GetProp("m_bInBuyZone") == 1 then
                GoShopping()
            end
        elseif event_name == "enter_buyzone" and not COND_BOUGHT_THIS_ROUND then
            GoShopping()
        end
    end
end)
